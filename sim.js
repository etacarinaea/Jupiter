...
var ts = parseFloat(localStorage.getItem("ts"));				// time scale // unit?
...
window.onload =function(){
	...
	setInterval(function(){
		...
		document.getElementById("ThemistoDIV").style.top = Yo -1 + YThemisto*vs*(I[4]/90) + "px";		// set "top"-value for Themisto
		
		document.getElementById("CurrentTime").innerHTML = "Day " + Math.round((t*60*1.769)*100/10000000)/100;	// display current time
	}, 10);
	
	// static objects
	document.getElementById("Jupiter").style.left = Xo -10 +"px";						// set "left"-value for Jupiter // -((S[7]*vs)/2)
	document.getElementById("Jupiter").style.top = Yo -10 +"px";						// set "top"-value for Jupiter // -((S[7]*vs)/2)
